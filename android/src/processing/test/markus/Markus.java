package processing.test.markus;
...
public class Markus extends PApplet {
...
public void inkPenSim(float px, float py, float x, float y) {
  ...
  if (py<=y) {
    ...
    factor = constrain(factor, 0.1f, 1);
...
public void penSim(float px, float py, float x, float y) {
  stroke(c);
  strokeWeight(2);
  strokeCap(SQUARE);
  line(px, py, x, y);
...
public void eraserSim(int px, int py, int x, int y) {
  stroke(255);
  strokeCap(ROUND);
  strokeWeight(20);
  line(px, py, x, y);
...
public void markerSim(int x, int y, int x2, int y2) {
  stroke(c, 10);
  strokeWeight(30);
  int w = x2 - x ;
  int h = y2 - y ;
...
public void paintbrushSim(float px, float py, float x, float y) {
  float speed = dist(px, py, x, y);
  float factor = map(speed, 0, 30, 5, 0.5f);
...
public void pencilSim(float px, float py, float x, float y) {
  stroke(c, 70);
  strokeWeight(1);
  strokeCap(SQUARE);
  line(px, py, x, y);
...
public void paint(int x, int y, int x2, int y2, int col, float alpha, float strk) {
  stroke(col, alpha);
  strokeWeight(strk);
  int w = x2 - x ;
  int h = y2 - y ;
